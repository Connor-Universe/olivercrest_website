...
<div class="wrapper">
        ...
        <div class="dash">
            <div class="container">
                ...
                <h3 class="first">
                          ...
                          <form action="remove_funds.php" method = "GET"  name="member">
                ...
                <div class="row">
                 ...
                 if(isset($_GET['success'])){
                  ...
                  <strong>Success</strong> Funds Successfully Removed
                </div> ";
                 }else{
                   $success = "";
                 }
    ...
    <thead>
        <tr>
            ...
            <th>Total Removed Funds(USD)</th>
            <th class="text-right">Remove Funds</th>
    ...
    $get_funds = "SELECT SUM(amount) AS value_adds FROM remove_funds where id_no = $id_no";
    ...
    <td class='text-right'>
       <form action='../include/remove.php' method='POST' id ='$id_no'>
         <input class='form-control' type='number' min='0' placeholder='Remove Funds' name='amount' required>
         <input type='hidden' name='id' value='$id_no'>
    
         <button type='button' rel='tooltip' class='btn btn-primary' data-toggle='modal' data-target='#exampleModal$id_no'> Remove</button> 
         <!-- start of modal -->
    ...
    <div class='modal-dialog' role='document'>
      <div class='modal-content'>
        <div class='modal-header'>
          <h5 class='modal-title'>Remove Funds!</h5>
          <button type='button' class='close' data-dismiss='modal' aria-hidden='true'>
            <i class='tim-icons icon-simple-remove'></i>
          </button>
        </div>
        <div class='modal-body'>
          <p>Are you sure you want to remove funds to this users account? You won't be able to undo this action! .</p>
        </div>
        <div class='modal-footer'>
       
          <button type='button' class='btn btn-secondary' data-dismiss='modal'>No</button>
          <button type='submit' class='btn btn-primary' name='remove'>Yes</button>
        </div>
      </div>
    </div>
  </div>
...
<ul class="pagination justify-content-center" style="padding-top:30px;">
                                 <?php
                                       ...
                                       if(isset($_GET['member'])){
                                        $member = $_GET['member'];
                                        $prev="<li class='page-item'><a class='page-link btn btn-danger' href='remove_funds.php?page=$previous&&member=$member&&member=#start'>Previous</a></li>";
                                        $reset="<li class='page-item'><a class='page-link btn btn-info' href='remove_funds.php'>Reset</a></li>";
                                        $nxt ="<li class='page-item'><a class='page-link btn btn-primary' href='remove_funds.php?page=$next&&member=$member&&member=#start'>Next</a></li>";
                                      }  else{
                                        $prev="<li class='page-item'><a class='page-link btn btn-danger' href='remove_funds.php?page=$previous#start'>Previous</a></li>";
                                        $reset="<li class='page-item'><a class='page-link btn btn-info' href='remove_funds.php'>Reset</a></li>";
                                        $nxt ="<li class='page-item'><a class='page-link btn btn-primary' href='remove_funds.php?page=$next#start'>Next</a></li>";
                                      }   
                                  if($page == 1){
                                    $prev = "<li class='page-item disabled'><a class='page-link btn btn-danger' href='remove_funds.php'>Previous</a></li>";
                                }
                                if($page == $pages OR $query_result == 0){
                                    $nxt="<li class='page-item disabled'><a class='page-link btn btn-primary' href='remove_funds.php?page=$next'>Next</a></li>";
                                }
                                 ?>
<?php echo"$prev";?>
<?php echo"$reset";?>
